...
class SimParamsConfig:
    target_dir: Path
    problem_key: problem_setups.ProblemSetup
...
def alfven_wave_circular_convergence_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            ...
            problem_key=problem_setups.ProblemSetup.ALFVEN_WAVE_CIRCULAR_CONVERGENCE,
            kwargs={
...
def alfven_wave_linear_convergence_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            target_dir=dataset_dir /
            f"{compute_scheme_key}-{averaging_scheme_key}-{reconstruction_order_key}",
            problem_key=problem_setups.ProblemSetup.ALFVEN_WAVE_LINEAR_CONVERGENCE,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": reconstruction_order_key,
...
def fast_wave_convergence_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            target_dir=dataset_dir /
            f"{compute_scheme_key}-{averaging_scheme_key}-{reconstruction_order_key}",
            problem_key=problem_setups.ProblemSetup.FAST_WAVE_CONVERGENCE,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": reconstruction_order_key,
...
def slow_wave_convergence_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            target_dir=dataset_dir /
            f"{compute_scheme_key}-{averaging_scheme_key}-{reconstruction_order_key}",
            problem_key=problem_setups.ProblemSetup.SLOW_WAVE_CONVERGENCE,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": reconstruction_order_key,
        ...
        for reconstruction_order_key in ("ppm", "ppm_ep"):
            configs.append(
                SimParamsConfig(
                    ...
                    problem_key=problem_setups.ProblemSetup.VORTEX_BALSARA,
                    kwargs={
                        "compute_scheme_key": "q26",
                        "averaging_scheme_key": "b25",
                        "reconstruction_order_key": reconstruction_order_key,
...
def blast_wave_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            ...
            problem_key=problem_setups.ProblemSetup.BLAST_WAVE,
            kwargs={
                "compute_scheme_key": "q26",
                "averaging_scheme_key": "b25",
...
def brio_wu_shock_tube_configs() -> list[SimParamsConfig]:
    ...
    configs: list[SimParamsConfig] = [
        SimParamsConfig(
            ...
            f"{compute_scheme_key}-{averaging_scheme_key}-{reconstruction_order_key}",
            problem_key=problem_setups.ProblemSetup.SHOCK_TUBE_BRIO_WU,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": reconstruction_order_key,
...
def ryu_jones_2a_shock_tube_configs() -> list[SimParamsConfig]:
    ...
    configs: list[SimParamsConfig] = [
        SimParamsConfig(
            ...
            f"{compute_scheme_key}-{averaging_scheme_key}-{reconstruction_order_key}",
            problem_key=problem_setups.ProblemSetup.SHOCK_TUBE_RYU_JONES_2A,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": reconstruction_order_key,
...
def current_sheet_configs() -> list[SimParamsConfig]:
    ...
    configs = [
        SimParamsConfig(
            ...
            problem_key=problem_setups.ProblemSetup.CURRENT_SHEET,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": "ppm",
...
def field_loop_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            ...
            problem_key=problem_setups.ProblemSetup.MAGNETIC_LOOP,
            kwargs={
                "compute_scheme_key": "q26",
                "averaging_scheme_key": "b25",
                "reconstruction_order_key": "ppm_ep",
...
def mhd_quirk_configs() -> list[SimParamsConfig]:
    ...
    return [
        SimParamsConfig(
            target_dir=dataset_dir / "q26-b25-ppm_ep",
            problem_key=problem_setups.ProblemSetup.CARBUNCLE,
            kwargs=kwargs,
        ),
        SimParamsConfig(
...
def orszag_tang_configs() -> list[SimParamsConfig]:
    ...
    configs: list[SimParamsConfig] = [
        SimParamsConfig(
            ...
            f"{compute_scheme_key}-{averaging_scheme_key}-{reconstruction_order_key}",
            problem_key=problem_setups.ProblemSetup.VORTEX_ORSZAG_TANG,
            kwargs={
                "compute_scheme_key": compute_scheme_key,
                "averaging_scheme_key": averaging_scheme_key,
                "reconstruction_order_key": reconstruction_order_key,
...
def main() -> None:
    for config in ALL_CONFIGS:
        builder = config.problem_key.build_sim_params
        sim_params = builder(**config.kwargs)
        sim_params.save_to_file(
